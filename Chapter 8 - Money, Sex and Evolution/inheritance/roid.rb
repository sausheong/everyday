class Roid
  attr_reader :velocity, :position, :uid, :sex, :lifespan, :age, :metabolism, :vision_range, :children
    ...
    if attractive and @sex == :female
      ...
      roids.each do |roid|
        if roid.attractive and roid.sex == :male
          baby = Roid.new(@slot, @position, @velocity, $uid_counter)
          $uid_counter += 1
  ...
  def pass_away
    unless @children.empty?
      inheritance = @energy/@children.size
      @children.each do |child|
        child.energy += inheritance
      end
    end
    $roids.delete self
  end
